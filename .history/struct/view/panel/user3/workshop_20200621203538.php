...
<script>
  ...
  function formCoachValidation() {
    ...
    var registeredTeacherTxt = $('#iptTopicWorkshop').find('option:selected').text();
    var registeredTeacherId = $('#iptTopicWorkshop').find('option:selected').val();
    printOk('#lblSelectTeacherType', '');
    if (registeredTeacherTxt.length == 0){
      printError('#lblRegisteredTeacher', 'لطفا موضوع کارگاه را وارد کنید');
      valid = 0;
    }else{
      iptNameTeacher = registeredTeacherTxt;
      iptIdTeacher = registeredTeacherId;
