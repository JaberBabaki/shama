...
<script>
  ...
  function formCoachValidation() {
    ...
    var iptTopicWorkshop = $('#iptEmployer').find('option:selected').text();
    var iptTopicWorkshopId = $('#iptEmployer').find('option:selected').val();
    printOk('#lblEmployer', '');
    if (iptTopicWorkshop.length == 0){
      printError('#lblEmployer', 'لطفا موضوع کارگاه را وارد کنید');
      status = false;
    }
