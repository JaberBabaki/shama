...
<script>
  $(function () {
    $('#IranMap .list li.province>ul>li>a').click(function (e) {
      ...
      if (provinceName) {
        $('#IranMap .city').html('نمایش مراکز مشاوره' + provinceName);
        var idCity=$(this).attr('at')
        var idOstan ="<?php echo $ResultData[0]['id']?>";
        window.location.replace('/UserCommon/showCoundiling/'+idCity+'/'+idOstan);
      }
      e.preventDefault();
    });
  });
  ...
  <div class='container'>
    <div id="IranMap" class="clear tac">
      ...
      <div class="list tac" style="width:100% ">
        <ul>
          <li class="province"><span>شهر های استان <?php echo $ResultData[0]['name'] ?></span>
            <ul>
              <?php
      ...
      <div class="city clear">
        <p>نمایش مراکز مشاوره با کلیک بر روی شهرها.</p>
      </div>
    </div>
  </div>
</section>