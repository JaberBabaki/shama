  ...
  <div class="page-container">
    <div class="page-content">
      ...
      <div class="sidebar sidebar-main sidebar-default" style="background-color: #C4C4C4 ">
                ...
                <?php endif;?>
						...
						<ul class="navigation ">
							...
							<li class="active"><a href="<?php echo baseUrl(); ?>/user1/notCompletedAppointments" class="text-right"><span>نوبت های آتی</span></a></li>
      ...
      <div class="content-wrapper">
				...
				<div class="content">
          <!-- Dropdown menu -->
          <?php if($notCompletedAppointments ==null): ?>
          ...
          <div class="table-responsive">
              ...
              <table class="table text-nowrap table-striped border table-hover table-condensed">
                <tbody>
                  ...
                  <?php for ($i=0; $i<count($notCompletedAppointments); $i++): ?>
                    <tr>
                      <td>
                        <div class="media-right">
                          <a href="#"><img src="/asset/image/per-pic/<?=$notCompletedAppointments[$i]['psychPhoto']?>" class="img-circle" alt="" style="height: 80px; width: 80px"></a>
                        </div>
                        <div class="media-left position">
                          <div class=""><a href="#" class="text-default text-semibold">دکتر <?=$notCompletedAppointments[$i]['psychName']?></a></div>
                          <div class="text-muted text-size-small">
                          <?=stringConverter($notCompletedAppointments[$i]['endTime'], 'enToFa')?> - <?=stringConverter($notCompletedAppointments[$i]['startTime'], 'enToFa')?> 
                          </div>
                        </div>
                      </td>
                      <td>
                        <div class="text-default text-semibold position">درمانگاه <?=$notCompletedAppointments[$i]['counselingName']?></div>
                      </td>
                      <td>
                        <div class="media-left position">
                          <div class="text-default text-semibold text-center"><?=dayNumToDayNameConverter($notCompletedAppointments[$i]['day']) ?></div>
                          <div class="text-muted text-size-small">
                            <?=dateConverter($notCompletedAppointments[$i]['date'], 'enToFa') ?>
                          </div>
                        </div>
                      </td>
                      <td><div class="text-default text-semibold position">
                        <?php if($notCompletedAppointments[$i]['paymentMode']==1): ?>
                          پرداخت در محل
                        <?php endif; ?>
                        <?php if($notCompletedAppointments[$i]['paymentMode']==2): ?>
                          پرداخت آنلاین
                        <?php endif; ?>
                      </div></td>
                      <td ><div id="nearest-appointment" onclick="runCancelDialog(<?=$notCompletedAppointments[$i]['appointment_id'] ?>)"  class="position shadow-lg"><button type="button" class="btn-lg btn-danger">لغو نوبت</button></div></td>
                    </tr>
                  <?php endfor; ?>    
                </tbody>
              </table>
