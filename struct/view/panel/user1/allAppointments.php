      ...
      <div class="sidebar sidebar-main sidebar-default" style="background-color: #C4C4C4 ">
                ...
                <?php endif;?>
						...
						<ul class="navigation ">
							...
							<li class="active"><a href="<?php echo baseUrl(); ?>/user1/allAppointments" class="text-right"><span>کل نوبت های رزرو شده</span></a></li>
            ...
            <?php if($allAppointments!=null): ?>
              <table class="table text-nowrap table-striped border table-hover table-condensed">
                <tbody>
                  ...
                  <?php for ($i=0; $i<count($allAppointments); $i++): ?>
                    <tr>
                      <td>
                        <div class="media-right">
                          <a href="#"><img src="/asset/image/per-pic/<?=$allAppointments[$i]['psychPhoto']?>" class="img-circle" alt="" style="height: 80px; width: 80px"></a>
                        </div>
                        <div class="media-left position">
                          <div class=""><a href="#" class="text-default text-semibold">دکتر <?=$allAppointments[$i]['psychName']?></a></div>
                          <div class="text-muted text-size-small">
                          <?=stringConverter($allAppointments[$i]['endTime'], 'enToFa')?> - <?=stringConverter($allAppointments[$i]['startTime'], 'enToFa')?> 
                          </div>
                        </div>
                      </td>
                      <td>
                        <div class="text-default text-semibold position">درمانگاه <?=$allAppointments[$i]['counselingName']?></div>
                      </td>
                      <td>
                        <div class="media-left position">
                          <div class="text-default text-semibold text-center"><?=dayNumToDayNameConverter($allAppointments[$i]['day']) ?></div>
                          <div class="text-muted text-size-small">
                            <?=dateConverter($allAppointments[$i]['date'], 'enToFa') ?>
                          </div>
                        </div>
                      </td>
                      <td><div class="text-default text-semibold position">
                        <?php if($allAppointments[$i]['paymentMode']==1): ?>
                          پرداخت در محل
                        <?php endif; ?>
                        <?php if($allAppointments[$i]['paymentMode']==2): ?>
                          پرداخت آنلاین
                        <?php endif; ?>
                      </div></td>
                    </tr>
